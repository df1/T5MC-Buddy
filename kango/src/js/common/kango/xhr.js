...
*/
kango.XHRRequest=function(){this.method='GET';this.url='';this.params={};this.headers={};this.async=true;this.contentType='';this.username='';this.password='';};kango.XHRResult=function(){this.response='';this.status=0;};kango.XHR=function(){};kango.XHR.prototype={_paramsToString:function(params){var result='';for(var key in params){if(params.hasOwnProperty(key)){if(result!=''){result+='&';}
result+=key+'='+params[key];}}
return result;},getXMLHttpRequest:function(){return new XMLHttpRequest();},send:function(details,callback){var req=this.getXMLHttpRequest();var method=details.method||'GET';var async=details.async||true;var params=details.params||null;var type=details.contentType||'text';var url=details.url;var username=details.username||'';var password=details.password||'';if(url.indexOf(kango.SCHEME)==0||(url.indexOf('http://')==-1&&url.indexOf('https://')==-1)){url=kango.io.getFileUrl(url);}
...
return getResult(req,type);}};kango.xhr=new kango.XHR();